...
async function assertCanEditJob(req: Request, _res: Response, next: NextFunction) {
  const user = (req as any).user as { id: number; role: "HR" | "LEAD" | "CANDIDATE" };
...
router.get("/", async (req, res, next) => {
  try {
    ...
    const { search,company,location,type,sort } = req.query as Record<string, string | undefined>;
...
router.get("/mine", auth, async (req, res, next) => {
  try {
    ...
    if (user.role === "LEAD") {
      ...
      const [data, total] = await Promise.all([
        prisma.jOB.findMany({ orderBy, skip, take, include: { jobOwner: true } }),
        prisma.jOB.count(),
      ]);
      return res.json({ data, page, pageSize, total, hasMore: skip + data.length < total });
    }
...
router.post("/", auth, requireRole("HR", "LEAD"), async (req, res, next) => {
  try {
    ...
    const job = await prisma.$transaction(async (tx) => {
      const created = await tx.jOB.create({
        data: { ...body, externalId },
      });
      await tx.jobOwner.create({
        data: { jobId: created.id, ownerId: (req as any).user.id },
      });
      return created;
    });
...
router.patch("/:id", auth, requireRole("HR", "LEAD"), assertCanEditJob, async (req, res, next) => {
  try {
    const id = Number(req.params.id);
    const data = updateJobSchema.parse(req.body);
    if ("externalId" in data) delete (data as any).externalId; 
...
router.delete("/:id", auth, requireRole("HR", "LEAD"), assertCanEditJob, async (req, res, next) => {
  try {
    const id = Number(req.params.id);
